...
namespace WebApplication.APIFORAPP
{
    ...
    public class DefaultController : ApiController
    {
        ...
        public HttpResponseMessage Active(ActiveReq active)
        {
            ...
            {
                result.Status = 1;
                result.Message = "Nhập Mã cào sản phẩm";
                result.Data = new List<ProductResult>();
                return ResponseMessage(result);
            }
            ...
            try
            {
                ...
                {
                    result.Status = 1;
                    result.Message = "Mã cào sản phẩm không tồn tại trong hệ thống";
                    result.Data = new List<ProductResult>();
                }
                else if (product.Status == 1)
                {
                    result.Status = 1;
                    result.Message = "Mã cào sản phẩm đã được kích hoạt trước đó";
                    result.Data = new List<ProductResult>();
                }

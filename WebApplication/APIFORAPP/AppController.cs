...
namespace WebApplication.APIFORAPP
{
    ...
    public class AppController : ApiController
    {
        ...
        public HttpResponseMessage GetProduct(string serial)
        {
            ...
            // lấy thông tin sản phẩm theo mã cào
                ...
                if (_phutung.Count() == 0)
                {
                    if (detailReq.Devices.Count() > 0)
                    {
                        foreach (var item in detailReq.Devices)
                        {
                            ...
                            var a_key = db.Accessary_Key.Where(a => a.Id_Key == warranti.Station_Warranti).FirstOrDefault(a => a.Name == item.Name);
                            var a_big = db.Accessary_Big.FirstOrDefault(a => a.Name == item.Name);
                            if (a_key!=null)
                            {
                                //cộng xuất kho, công nhập kho ở trạm
                                a_key.CountExport = a_key.CountExport + item.Quantity??1;
                                db.Entry(a_key).State = EntityState.Modified;
                                //cộng xuất kho ở tổng
                                a_big.CountExport = a_big.CountExport + item.Quantity??1;
                                db.Entry(a_big).State = EntityState.Modified;
                            }
                            else
                            {
                                //thêm linh kiện vào trạm
                                //cộng xuất kho, công nhập kho ở trạm
                                var new_key = new Accessary_Key()
                                {
                                    Id_Key = warranti.Station_Warranti,
                                    Name = item.Name,
                                    Price = a_big.Price,
                                    CountExport = item.Quantity ??1,
                                    Code = a_big.Code
                                };
                                db.Accessary_Key.Add(new_key);
                            ...
                            var logkey = new Acessary_Log_Key()
                            {
                                ...
                                Createdate = DateTime.Now,
                                Id_Akey = a_big.Id.ToString(),
                                Type = 2//xuat
                            };
                            db.Acessary_Log_Key.Add(logkey);
                            // thay đổi trạng thái cho phiếu 
        ...
        {
            var product = db.Products.FirstOrDefault(a => a.Serial == serial);
            if (product == null)
            {
                var result = new ResultString()
                {
            ...
            if (!string.IsNullOrEmpty(product.AgentC1))
            {
                ...
                {
                    Status = -1,
                    Message = string.Format("Serial {0} đã được xuất trước đó", serial),
                    Data = new List<string>()
                };
                return ResponseMessage(result);
            }
        ...
        */
        ...
        public HttpResponseMessage Import_Product(Import_Product model)
        {
            ...
            try
            {
                foreach (var item in model.Items)
                {
                    var check_product = db.Products.FirstOrDefault(a => a.Code == item.Serial || a.Serial == item.Serial);
                    if (check_product != null)
                    {
                        ...
                        {
                            Status = -1,
                            Message = "Serial " + item.Serial + " sản phẩm đã tồn tại trong hệ thông.",
                            Data = new List<string>()
                        };
                        return ResponseMessage(resulter);
                    }
                ...
                {
                    Status = 1,
                    Message = "Đã tạo phiếu xuất thành công. Mã phiếu xuất là : " + import.Code,
                    Data = new List<string>()
                };
                return ResponseMessage(result);
            }

    ...
    public class ProductController : BaseController
    {
            ...
            if ((file != null) && (file.ContentLength > 0) && !string.IsNullOrEmpty(file.FileName))
            {
                ...
                using (var package = new ExcelPackage(file.InputStream))
                {
                    ...
                    for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                    {
                        ...
                        try { limited = workSheet.Cells[rowIterator, 7].Value?.ToString(); } catch (Exception) { limited = ""; }
                        try { exportdate = workSheet.Cells[rowIterator, 8].Value?.ToString(); } catch (Exception) { exportdate = null; }
                        try { ag_phone = workSheet.Cells[rowIterator, 9].Value?.ToString(); } catch (Exception) { ag_phone = ""; }
                        try { note = workSheet.Cells[rowIterator, 10].Value?.ToString(); } catch (Exception) { note = ""; }
                        try { waitday = workSheet.Cells[rowIterator, 11].Value?.ToString(); } catch (Exception) { waitday = ""; }
                        try { ag_name = workSheet.Cells[rowIterator, 12].Value?.ToString(); } catch (Exception) { ag_name = ""; }
                        try { ag_adr = workSheet.Cells[rowIterator, 13].Value?.ToString(); } catch (Exception) { ag_adr = ""; }
                        try { serialBrand = workSheet.Cells[rowIterator, 14].Value?.ToString(); } catch (Exception) { serialBrand = ""; }
        ...
        public ActionResult CreateConfirm([Bind(Include = "")] Product product)
        {
            if (ModelState.IsValid)
            {
                ...
                product.SerialBrand = product.SerialBrand;
                db.Products.Add(product);
                db.SaveChanges();
        ...
        public void Sanpham()
        {
            ...
            Sheet.Cells["T1"].Value = "serial hãng";
            Sheet.Cells["U1"].Value = "ĐL/ KH xuất";
            Sheet.Cells["V1"].Value = "Đ/c đại lý/ KH";
            Sheet.Cells["W1"].Value = "SĐT đại lý/ KH";
            ...
            foreach (var item in listexc)
            {
                ...
                Sheet.Cells[string.Format("U{0}", row)].Value = item.Ag_Name;
                Sheet.Cells[string.Format("V{0}", row)].Value = item.Ag_Adr;
                Sheet.Cells[string.Format("W{0}", row)].Value = item.Ag_Phone;
                row++;
            }
